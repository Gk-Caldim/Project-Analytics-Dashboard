...
import { ChevronDown, ChevronRight, AlertTriangle, AlertCircle, CheckCircle, Clock, Calendar, Users, Wallet, LayoutGrid, TableProperties, BarChart3 } from 'lucide-react';
...
const PortfolioHealthMatrix = ({
  ...
  const healthDonutOption = useMemo(() => ({
    ...
    tooltip: { trigger: 'item', formatter: '{b}: {c} projects ({d}%)', backgroundColor: 'var(--surface)', borderColor: 'var(--border-strong)', textStyle: { color: 'var(--text-primary)', fontSize: 11, fontFamily: 'Inter, sans-serif' } },
    legend: { show: false },
    series: [{
      type: 'pie',
      radius: ['52%', '80%'],
  ...
  const delayBarOption = useMemo(() => {
    ...
    return {
      ...
      xAxis: { type: 'value', axisLabel: { color: 'var(--text-secondary)', fontSize: 10, fontFamily: 'Inter, sans-serif' }, splitLine: { lineStyle: { color: 'var(--border-subtle)', type: 'dashed' } }, axisLine: { show: false } },
      yAxis: { type: 'category', data: sorted.map(p => p.project_name?.split(' ').slice(0, 2).join(' ') || p.project_name), axisLabel: { color: 'var(--text-secondary)', fontSize: 10, fontFamily: 'Inter, sans-serif', width: 90, overflow: 'truncate' }, axisLine: { show: false }, axisTick: { show: false } },
      series: [{
        ...
        data: sorted.map(p => ({
          value: p.delayed || 0,
          itemStyle: { color: (p.delayed || 0) > 5 ? '#ef4444' : (p.delayed || 0) > 2 ? '#f59e0b' : '#6366f1', borderRadius: [0, 3, 3, 0] }
        }))
      }]
    };
  }, [projectsSummary]);
  ...
  const scheduleStackOption = useMemo(() => {
    ...
    return {
      backgroundColor: 'transparent',
      tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, backgroundColor: 'var(--surface)', borderColor: 'var(--border-strong)', textStyle: { color: 'var(--text-primary)', fontSize: 11, fontFamily: 'Inter, sans-serif' } },
      legend: { data: ['On Track', 'Delayed', 'Pending'], bottom: 0, textStyle: { color: 'var(--text-secondary)', fontSize: 10, fontFamily: 'Inter, sans-serif' }, itemWidth: 10, itemHeight: 10 },
      grid: { left: 8, right: 8, top: 8, bottom: 28, containLabel: true },
      xAxis: { type: 'category', data: projects.map(p => p.project_name?.split(' ').slice(0, 2).join(' ') || ''), axisLabel: { color: 'var(--text-secondary)', fontSize: 9, rotate: 25, fontFamily: 'Inter, sans-serif' }, axisLine: { show: false }, axisTick: { show: false } },
      yAxis: { type: 'value', max: 100, axisLabel: { color: 'var(--text-secondary)', fontSize: 9, formatter: '{value}%', fontFamily: 'Inter, sans-serif' }, splitLine: { lineStyle: { color: 'var(--border-subtle)', type: 'dashed' } }, axisLine: { show: false } },
      series: [
        { name: 'On Track', type: 'bar', stack: 'total', barMaxWidth: 20, itemStyle: { color: '#10b981', borderRadius: 0 }, data: projects.map(p => Math.round(p.on_track_pct || 0)) },
        { name: 'Delayed', type: 'bar', stack: 'total', barMaxWidth: 20, itemStyle: { color: '#ef4444', borderRadius: 0 }, data: projects.map(p => Math.round(p.delay_pct || 0)) },
        { name: 'Pending', type: 'bar', stack: 'total', barMaxWidth: 20, itemStyle: { color: '#64748b', borderRadius: [3, 3, 0, 0] }, data: projects.map(p => Math.max(0, 100 - Math.round(p.on_track_pct || 0) - Math.round(p.delay_pct || 0))) },
      ]
    };
  }, [projectsSummary]);
  ...
  const renderAnalyticsMode = () => {
    ...
    const totalDelayed = projectsSummary.reduce((a, p) => a + (p.delayed || 0), 0);
    const avgDelay = totalProjects > 0 ? (projectsSummary.reduce((a, p) => a + (p.avg_delay_days || 0), 0) / totalProjects).toFixed(1) : 0;
    ...
    return (
      <div className="p-3 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            { label: 'Total Projects', value: totalProjects, color: 'text-[var(--text-primary)]' },
            { label: 'Critical (Red)', value: redCount, color: 'text-rose-500' },
            { label: 'At Risk (Yellow)', value: yellowCount, color: 'text-amber-500' },
            { label: 'Avg Delay', value: `${avgDelay}d`, color: 'text-[var(--text-secondary)]' },
          ].map(kpi => (
        ...
        <div className="grid grid-cols-12 gap-3">
          ...
          <div className="col-span-3 bg-[var(--bg)] border border-[var(--border-subtle)] rounded p-2 flex flex-col">
            <span className="text-[9px] uppercase tracking-wider font-bold text-[var(--text-muted)] mb-1">Health Distribution</span>
            ...
            <div className="flex flex-col gap-1 mt-1">
              {[{c:'#ef4444',l:'Critical',v:redCount},{c:'#f59e0b',l:'At Risk',v:yellowCount},{c:'#10b981',l:'On Track',v:greenCount}].map(i=>(
                <div key={i.l} className="flex items-center justify-between text-[9px]">
                  <span className="flex items-center gap-1"><span className="size-2 rounded-full shrink-0" style={{background:i.c}} /><span className="text-[var(--text-secondary)]">{i.l}</span></span>
                  <span className="font-bold text-[var(--text-primary)]">{i.v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="col-span-4 bg-[var(--bg)] border border-[var(--border-subtle)] rounded p-2 flex flex-col">
            <span className="text-[9px] uppercase tracking-wider font-bold text-[var(--text-muted)] mb-1">Delayed Milestones by Project</span>
            <ReactECharts option={delayBarOption} style={{ height: 160, width: '100%' }} opts={{ renderer: 'svg' }} />
          </div>

          {/* Schedule Stack */}
          <div className="col-span-5 bg-[var(--bg)] border border-[var(--border-subtle)] rounded p-2 flex flex-col">
            <span className="text-[9px] uppercase tracking-wider font-bold text-[var(--text-muted)] mb-1">Schedule Status Distribution (%)</span>
            <ReactECharts option={scheduleStackOption} style={{ height: 160, width: '100%' }} opts={{ renderer: 'svg' }} />
          </div>
        </div>
        ...
        <div className="border border-[var(--border-subtle)] rounded overflow-hidden">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-[var(--elevated-card)] border-b border-[var(--border-subtle)] text-[var(--text-secondary)] font-bold uppercase tracking-wider text-[9px]">
                <th className="px-3 py-1.5">Project</th>
                <th className="px-3 py-1.5 text-center">Health</th>
                <th className="px-3 py-1.5 text-center">Completed</th>
                <th className="px-3 py-1.5 text-center">Delayed</th>
                <th className="px-3 py-1.5 text-center">Pending</th>
                <th className="px-3 py-1.5 text-right">Avg Delay</th>
            ...
            <tbody>
              {projectsSummary.map(proj => (
                <tr key={proj.project_id} className="border-b border-[var(--border-subtle)] last:border-0 hover:bg-[var(--table-hover)] transition-colors">
                  <td className="px-3 py-1.5 font-semibold text-[var(--text-primary)] max-w-[160px] truncate">{proj.project_name}</td>
                  <td className="px-3 py-1.5 text-center">
                    <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold border ${getHealthColor(proj.project_health)}`}>{proj.project_health || '—'}</span>
                  </td>
                  <td className="px-3 py-1.5 text-center font-bold text-emerald-500">{proj.completed}</td>
                  <td className="px-3 py-1.5 text-center font-bold text-rose-500">{proj.delayed}</td>
                  <td className="px-3 py-1.5 text-center font-bold text-[var(--text-secondary)]">{proj.pending}</td>
                  <td className="px-3 py-1.5 text-right font-semibold text-[var(--text-primary)]">{proj.avg_delay_days || 0}d</td>
                  <td className="px-3 py-1.5 text-center">
                    <button onClick={() => onProjectSelect(proj)} className="px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-blue-500 hover:text-blue-600 border border-blue-500/20 hover:border-blue-500/40 hover:bg-blue-500/5 rounded transition-all cursor-pointer">Open</button>
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 p-4">
        {projectsSummary.map(proj => {
          ...
          const balance = proj.budgetBalance || 0;
          ...
          return (
            ...
            >
              ...
              <div className="flex justify-between items-start mb-3 gap-2">
                ...
                <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase border shrink-0 ${getHealthColor(proj.project_health)}`}>
                  {proj.project_health}
                </span>
              </div>

              {/* Milestones Summary & Stacked Progress Bar */}
              <div className="flex flex-col gap-2 mb-3">
                <div className="flex justify-between items-center text-[10px] text-[var(--text-secondary)]">
                  <span>Milestones Progress</span>
                  <span className="font-extrabold text-[var(--text-primary)]">{proj.on_track_pct}%</span>
                </div>

                {/* Stacked Schedule Timeline Chart */}
                <div className="h-2.5 bg-[var(--bg)] rounded-full overflow-hidden flex border border-[var(--border-subtle)]">
                  <div
                    className="h-full bg-emerald-500 transition-all duration-500"
                    style={{ width: `${proj.on_track_pct}%` }}
                    title="On Track"
                  />
                  <div
                    className="h-full bg-rose-500 transition-all duration-500"
                    style={{ width: `${proj.delay_pct || 0}%` }}
                    title="Delayed"
                  />
                  <div
                    className="h-full bg-[var(--border-strong)] opacity-40 transition-all duration-500"
                    style={{ width: `${100 - proj.on_track_pct - (proj.delay_pct || 0)}%` }}
                    title="Pending"
                  />
                </div>
              ...
              <div className="border-t border-[var(--border-subtle)] pt-2.5 mb-3 flex flex-col gap-1.5">
                <span className="text-[9px] uppercase tracking-wider font-extrabold text-[var(--text-muted)] flex items-center justify-between">
                  <span>Critical Issues ({projectIssues.length})</span>
                  {projectIssues.length > 0 && <span className="size-2 rounded-full bg-rose-500 animate-pulse" />}
                </span>
              ...
              <div className="border-t border-[var(--border-subtle)] pt-2.5 flex flex-col gap-1.5">
                <div className="flex justify-between items-center text-[9px] uppercase tracking-wider font-extrabold text-[var(--text-muted)]">
                  <span>Budget Allocation</span>
                  <span className="font-extrabold text-[var(--text-primary)]">{utilizationPct}% Spent</span>
                </div>
                <div className="flex justify-between font-bold text-[var(--text-secondary)] text-[10px] my-0.5">
                  <span>Spent: ${utilized.toLocaleString()}</span>
                  <span>Approved: ${approved.toLocaleString()}</span>
                </div>
                ...
                <React.Fragment key={proj.project_id}>
                  ...
                  <tr className={`border-b border-[var(--border-subtle)] last:border-0 hover:bg-[var(--table-hover)] transition-colors ${isExpanded ? 'bg-[var(--table-hover)]' : ''}`}>
                    ...
                    <td className="p-3 font-semibold text-[var(--text-primary)]">
                      <button
                        onClick={() => onProjectSelect(proj)}
                        className="hover:underline hover:text-blue-500 text-left font-bold cursor-pointer"
                      >
                        {proj.project_name}
                      </button>
                    </td>
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2 max-w-[120px] mx-auto">
                        ...
                        <span className="text-[10px] font-bold text-[var(--text-secondary)]">{proj.on_track_pct}%</span>
                      </div>
                    </td>
                    <td className="p-3 text-center font-bold text-emerald-500">{proj.completed}</td>
                    <td className="p-3 text-center font-bold text-rose-500">{proj.delayed}</td>
                    <td className="p-3 text-center font-bold text-[var(--text-secondary)]">{proj.pending}</td>
                    <td className="p-3 text-right font-semibold text-[var(--text-primary)]">{proj.avg_delay_days || 0}d</td>
                    <td className="p-3 text-center">
                      <button
                        onClick={() => onProjectSelect(proj)}
                    ...
                    <tr className="bg-[var(--bg)]/40 border-b border-[var(--border-strong)]">
                      <td colSpan="9" className="p-4">
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 text-[var(--text-primary)]">
                          ...
                          {/* Col 1: Schedule Stacked Timeline & Datasets */}
                          <div className="border border-[var(--border-subtle)] bg-[var(--surface)] p-3 rounded-lg flex flex-col gap-2">
                            <h4 className="m-0 text-[10px] font-extrabold uppercase tracking-wider text-[var(--text-secondary)] flex items-center gap-1.5 border-b border-[var(--border-subtle)] pb-2">
                            ...
                            <div className="flex flex-col gap-1 my-1">
                              <span className="text-[9px] uppercase tracking-wider font-extrabold text-[var(--text-muted)]">Milestone status allocation</span>
                            ...
                            {projectIssues.length > 0 ? (
                              <div className="flex flex-col gap-1.5 mt-1 overflow-y-auto max-h-[120px]">
                                {projectIssues.slice(0, 3).map(issue => (
                                  <div
                                    key={issue.id}
                                    className="text-[11px] bg-[var(--bg)] p-2 rounded border border-[var(--border-subtle)] flex flex-col gap-1"
                                  >
                          ...
                          {/* Col 3: Budgets & Capex Allocation */}
                          <div className="border border-[var(--border-subtle)] bg-[var(--surface)] p-3 rounded-lg flex flex-col gap-2">
                            <h4 className="m-0 text-[10px] font-extrabold uppercase tracking-wider text-[var(--text-secondary)] flex items-center gap-1.5 border-b border-[var(--border-subtle)] pb-2">
                            ...
                            <div className="flex flex-col gap-2.5 mt-1 text-[11px]">
                              <div className="flex justify-between border-b border-[var(--border-subtle)] pb-1.5">
                                <span className="text-[var(--text-secondary)]">Capex Approved:</span>
                                <span className="font-bold text-[var(--text-primary)]">
                                  ${(proj.budgetApproved || projectBudget?.overall_budget || 0).toLocaleString()}
                                </span>
                              </div>
                              <div className="flex justify-between border-b border-[var(--border-subtle)] pb-1.5">
                                <span className="text-[var(--text-secondary)]">Spent Utilization:</span>
                                <span className="font-bold text-emerald-500">
                                  ${(proj.budgetUtilized || 0).toLocaleString()}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-[var(--text-secondary)]">Balance reserve:</span>
                                <span className="font-bold text-amber-500">
                                  ${(proj.budgetBalance || 0).toLocaleString()}
                                </span>
                              </div>
                            </div>
                          </div>
