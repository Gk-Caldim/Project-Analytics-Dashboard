/**
 * IssuesTab.jsx — Real-data Issues & NCR Tab (Recharts version)
 */
...
export default function IssuesTab({ allIssues = [], issuesLoading, isError, refetchIssues }) {
  ...
  const filtered = useMemo(() => {
    return allIssues.filter(i => {
      ...
      if (search.trim()) {
        ...
        return (i.title || '').toLowerCase().includes(q) || (i.owner || '').toLowerCase().includes(q);
      }
      return true;
    });
  ...
  const high   = allIssues.filter(i => i.priority === 'High' && i.status !== 'Closed').length;

  // Data for NCR Severity Recharts Stacked Bar
  ...
  // Data for Resolution Status Recharts Pie
  ...
  return (
    <div className="ah-fade-up">
      <div className="ah-section-header">
        ...
        <h2 className="ah-section-title">Issues & NCR Tracker</h2>
      </div>
        ...
        <div className="ah-chart-grid ah-chart-grid-2" style={{ marginBottom: 16 }}>
          <div className="ah-card">
            <div className="ah-card-header">
              <h3 className="ah-card-title">NCR Severity & Status</h3>
            </div>
                    ...
                    paginated.map(issue => (
                      <tr key={issue.id}>
                        ...
                        <td style={{ color: 'var(--ah-text-secondary)', fontWeight: 600, fontSize: 11 }}>
                          {issue.project_name || `Proj ${issue.project_id}`}
                        </td>
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderTop: '1px solid var(--ah-border)' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  >← Prev</button>
                  <span style={{ padding: '4px 12px', fontSize: 12, color: 'var(--ah-text-secondary)' }}>
                    {page} / {totalPages}
                  </span>
                  <button
