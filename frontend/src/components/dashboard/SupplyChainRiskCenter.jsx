...
const SupplyChainRiskCenter = ({
  ...
  const runImpactAnalysis = async (project) => {
    ...
    try {
      const res = await API.get(`/budget/proposal/${encodeURIComponent(project.project_name)}`, { params: { industry: 'Automotive Manufacturing', procurement_categories: activeCommodity, currency: 'USD' } });
  ...
  // Real sparkline data from current_price / previous_price
  const sparklineOption = useMemo(() => {
    ...
    // Build a 5-point mini trend from prev → curr using volatility to add midpoints
    ...
    return {
      ...
      series: [{ type: 'line', data: pts, smooth: true, showSymbol: false, lineStyle: { color: isUp ? '#ef4444' : '#10b981', width: 1.5 }, areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: isUp ? 'rgba(239,68,68,0.18)' : 'rgba(16,185,129,0.18)' }, { offset: 1, color: 'transparent' }] } } }]
    };
  }, [activeCommodity, commodityPrices]);

  // Risk comparison: all commodities — volatility_index, procurement_risk, supply_chain_risk
  const riskComparisonOption = useMemo(() => {
    ...
    return {
      ...
      legend: { data: ['Volatility', 'Procurement Risk', 'Supply Chain Risk'], bottom: 0, textStyle: { color: 'var(--text-secondary)', fontSize: 9, fontFamily: 'Inter, sans-serif' }, itemWidth: 10, itemHeight: 10 },
      grid: { left: 8, right: 8, top: 8, bottom: 30, containLabel: true },
      xAxis: { type: 'category', data: categories.map(c => c.charAt(0).toUpperCase() + c.slice(1)), axisLabel: { color: 'var(--text-secondary)', fontSize: 9, fontFamily: 'Inter, sans-serif', rotate: 20 }, axisLine: { show: false }, axisTick: { show: false } },
      yAxis: { type: 'value', max: 1, axisLabel: { color: 'var(--text-secondary)', fontSize: 9, formatter: v => (v * 100).toFixed(0) + '%', fontFamily: 'Inter, sans-serif' }, splitLine: { lineStyle: { color: 'var(--border-subtle)', type: 'dashed' } }, axisLine: { show: false } },
      series: [
        { name: 'Volatility', type: 'bar', barMaxWidth: 12, itemStyle: { color: '#f59e0b', borderRadius: 0 }, data: categories.map(c => +(commodityPrices[c]?.volatility_index || 0).toFixed(3)) },
        { name: 'Procurement Risk', type: 'bar', barMaxWidth: 12, itemStyle: { color: '#ef4444', borderRadius: 0 }, data: categories.map(c => +(commodityPrices[c]?.procurement_risk || 0).toFixed(3)) },
        { name: 'Supply Chain Risk', type: 'bar', barMaxWidth: 12, itemStyle: { color: '#6366f1', borderRadius: [3, 3, 0, 0] }, data: categories.map(c => +(commodityPrices[c]?.supply_chain_risk || 0).toFixed(3)) },
      ]
    };
  ...
  const priceChangeOption = useMemo(() => {
    ...
    return {
      backgroundColor: 'transparent',
      tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, backgroundColor: 'var(--surface)', borderColor: 'var(--border-strong)', textStyle: { color: 'var(--text-primary)', fontSize: 11, fontFamily: 'Inter, sans-serif' }, formatter: p => `${p[0].name}: ${p[0].value >= 0 ? '+' : ''}${p[0].value}%` },
      grid: { left: 8, right: 16, top: 8, bottom: 4, containLabel: true },
      xAxis: { type: 'value', axisLabel: { color: 'var(--text-secondary)', fontSize: 9, formatter: v => `${v >= 0 ? '+' : ''}${v}%`, fontFamily: 'Inter, sans-serif' }, splitLine: { lineStyle: { color: 'var(--border-subtle)', type: 'dashed' } }, axisLine: { show: false } },
      yAxis: { type: 'category', data: sorted.map(c => c.charAt(0).toUpperCase() + c.slice(1)), axisLabel: { color: 'var(--text-secondary)', fontSize: 10, fontFamily: 'Inter, sans-serif' }, axisLine: { show: false }, axisTick: { show: false } },
      series: [{ type: 'bar', barMaxWidth: 14, data: pcts.map(v => ({ value: v, itemStyle: { color: v >= 0 ? '#ef4444' : '#10b981', borderRadius: v >= 0 ? [0, 3, 3, 0] : [3, 0, 0, 3] } })) }]
    };
  }, [categories, commodityPrices]);
  ...
  const renderAnalytics = () => {
    ...
    const highRisk = categories.filter(c => (commodityPrices[c]?.supply_chain_risk || 0) > 0.6 || (commodityPrices[c]?.percentage_change || 0) > 10).length;
    const avgVol = categories.length > 0 ? (categories.reduce((a, c) => a + (commodityPrices[c]?.volatility_index || 0), 0) / categories.length * 100).toFixed(1) : 0;
    const topRiser = categories.reduce((a, c) => (commodityPrices[c]?.percentage_change || 0) > (commodityPrices[a]?.percentage_change || 0) ? c : a, categories[0]);
    ...
    return (
      <div className="p-3 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            { label: 'Tracked Commodities', value: categories.length, color: 'text-[var(--text-primary)]' },
            { label: 'High Risk', value: highRisk, color: 'text-rose-500' },
            { label: 'Avg Volatility', value: `${avgVol}%`, color: 'text-amber-500' },
            { label: 'Top Riser', value: topRiser ? topRiser.charAt(0).toUpperCase() + topRiser.slice(1) : '—', color: 'text-rose-500' },
          ].map(kpi => (
        ...
        <div className="grid grid-cols-12 gap-3">
          <div className="col-span-5 bg-[var(--bg)] border border-[var(--border-subtle)] rounded p-2 flex flex-col">
            <span className="text-[9px] uppercase tracking-wider font-bold text-[var(--text-muted)] mb-1">Risk Comparison (Volatility · Procurement · Supply Chain)</span>
            <ReactECharts option={riskComparisonOption} style={{ height: 160, width: '100%' }} opts={{ renderer: 'svg' }} />
          </div>
          <div className="col-span-4 bg-[var(--bg)] border border-[var(--border-subtle)] rounded p-2 flex flex-col">
            <span className="text-[9px] uppercase tracking-wider font-bold text-[var(--text-muted)] mb-1">Price Change Ranking</span>
            <ReactECharts option={priceChangeOption} style={{ height: 160, width: '100%' }} opts={{ renderer: 'svg' }} />
          </div>
          <div className="col-span-3 bg-[var(--bg)] border border-[var(--border-subtle)] rounded p-2 flex flex-col gap-1">
            <span className="text-[9px] uppercase tracking-wider font-bold text-[var(--text-muted)]">Spot Prices</span>
            <div className="flex flex-col gap-1 overflow-y-auto">
              {categories.map(cat => {
                const d = commodityPrices[cat];
                const isUp = (d?.percentage_change || 0) >= 0;
                return (
                  <button key={cat} onClick={() => { setSelectedCommodity(cat); setViewMode('detail'); }}
                    className={`flex justify-between items-center px-2 py-1 rounded border text-[10px] transition-all cursor-pointer hover:bg-[var(--table-hover)] ${activeCommodity === cat ? 'border-blue-500/40 bg-[var(--table-hover)]' : 'border-[var(--border-subtle)] bg-transparent'}`}>
                    <span className="font-bold text-[var(--text-primary)] uppercase">{cat}</span>
                    <div className="flex items-center gap-1.5">
                      <span className="font-semibold text-[var(--text-secondary)]">${(d?.current_price || 0).toFixed(1)}</span>
                      <span className={`font-bold text-[9px] ${isUp ? 'text-rose-500' : 'text-emerald-500'}`}>{isUp ? '+' : ''}{(d?.percentage_change || 0).toFixed(1)}%</span>
                    </div>
                  </button>
                );
              })}
  ...
  const renderDetail = () => (
    <div className="grid grid-cols-1 lg:grid-cols-12 min-h-0 divide-y lg:divide-y-0 lg:divide-x divide-[var(--border-subtle)] bg-[var(--surface)]">
      {/* Col 1: Commodity List with real sparklines */}
      <div className="lg:col-span-4 p-3 overflow-y-auto max-h-[300px] flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1.5">
          {categories.map(cat => {
            ...
            const curr = data?.current_price || 0;
            const vol = data?.volatility_index || 0.05;
            const pts = [prev, prev + (curr - prev) * 0.25 + prev * vol * 0.3, prev + (curr - prev) * 0.5 - prev * vol * 0.2, prev + (curr - prev) * 0.75 + prev * vol * 0.1, curr].map(v => parseFloat(v.toFixed(2)));
            const miniOption = {
            ...
            return (
              <div key={cat} onClick={() => { setSelectedCommodity(cat); setAnalyzingProject(null); setAnalysisResult(null); }}
                className={`p-2 rounded border transition-all cursor-pointer flex justify-between items-center hover:bg-[var(--table-hover)] ${isSelected ? 'bg-[var(--table-hover)] border-blue-500/40 shadow-sm' : 'bg-[var(--surface)] border-[var(--border-subtle)]'}`}>
                <div className="flex flex-col gap-0.5 min-w-0">
                  <span className="font-bold text-[var(--text-primary)] text-[11px] uppercase truncate">{cat}</span>
                  <span className="text-[9px] text-[var(--text-muted)] truncate">SCR: {((data?.supply_chain_risk || 0) * 100).toFixed(0)}%</span>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <ReactECharts option={miniOption} style={{ height: 28, width: 60 }} opts={{ renderer: 'svg' }} />
                  <div className="text-right min-w-[52px]">
                    <div className="font-bold text-[var(--text-primary)] text-[10px]">${(data?.current_price || 0).toFixed(1)}</div>
                    <div className={`text-[9px] font-bold flex items-center gap-0.5 justify-end ${isUp ? 'text-rose-500' : 'text-emerald-500'}`}>
        ...
        {impactedProjects.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {impactedProjects.map(proj => {
              ...
              return (
                <div key={proj.project_id} onClick={() => runImpactAnalysis(proj)}
                  className={`p-2 rounded border transition-all cursor-pointer flex flex-col gap-1 hover:bg-[var(--table-hover)] ${isSelected ? 'bg-[var(--table-hover)] border-blue-500/40' : 'bg-[var(--surface)] border-[var(--border-subtle)]'}`}>
                  <div className="flex justify-between items-start gap-2">
                    <span className="font-semibold text-[var(--text-primary)] text-[11px] truncate">{proj.project_name}</span>
                    <span className={`text-[8px] px-1.5 py-0.5 border rounded uppercase shrink-0 ${getHealthColor(proj.current_health)}`}>{proj.current_health}</span>
                  </div>
      ...
      {/* Col 3: Risk chart when idle, analysis when project selected */}
      <div className="lg:col-span-4 p-3 flex flex-col gap-2">
        <h4 className="m-0 text-[var(--text-secondary)] uppercase tracking-wider text-[9px] font-bold select-none">Volatility Assessment</h4>
        {loadingAnalysis ? (
          ...
          <div className="flex flex-col gap-1 flex-1">
            <span className="text-[9px] uppercase tracking-wider font-bold text-[var(--text-muted)]">Risk Profile — {activeCommodity}</span>
          ...
          <div className="flex-1 flex flex-col gap-3 text-[11px]">
            <div className="bg-[var(--surface)] border border-[var(--border-subtle)] p-2.5 rounded flex flex-col gap-1.5 shadow-sm">
              ...
              {analysisResult ? (
                <div className="flex flex-col gap-1.5 text-[11px] mt-1">
                  ...
                  <div className="flex justify-between border-b border-[var(--border-subtle)] pb-1.5">
                    <span className="text-[var(--text-secondary)]">Material Buffer:</span>
                    <span className="font-bold text-rose-500">+${(analysisResult.reconciliation?.raw_material_risk_buffer || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--text-secondary)]">Suggested Budget:</span>
                    <span className="font-bold text-[var(--text-primary)]">${(analysisResult.suggested_budget_usd || 0).toLocaleString()}</span>
                  </div>
                </div>
              ) : (
            ...
            {analysisResult && (
              <button onClick={async () => {
                try {
                  await API.post('/budget/revisions', { project_id: String(analyzingProject.project_id), project_name: analyzingProject.project_name, pm_name: 'System Analysis', previous_budget: analysisResult.original_budget_usd, revised_budget: analysisResult.suggested_budget_usd, reasons: `AI Procurement Buffer proposal for category: ${activeCommodity}.` });
    ...
    <div className="w-full bg-[var(--surface)] border border-[var(--border-strong)] rounded-lg overflow-hidden flex flex-col shadow-sm">
      <div className="px-4 py-2.5 border-b border-[var(--border-subtle)] bg-[var(--surface)] flex justify-between items-center shrink-0">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-[var(--text-secondary)] bg-[var(--bg)] px-2 py-0.5 rounded border border-[var(--border-subtle)] font-bold">{categories.length} Commodities</span>
          <div className="flex border border-[var(--border-subtle)] rounded overflow-hidden">
            ...
            <button onClick={() => setViewMode('detail')} className={`px-2.5 py-1 flex items-center gap-1 text-[9px] font-bold uppercase cursor-pointer border-0 outline-none transition-colors ${viewMode === 'detail' ? 'bg-blue-600 text-white' : 'bg-[var(--surface)] text-[var(--text-secondary)] hover:bg-[var(--table-hover)]'}`}>
