...
import { 
  BarChart3, FileUp, Database, MessageSquare as MOMIcon, Settings as SettingsIcon, 
  Globe, Box, Layers 
} from 'lucide-react';
...
const ProjectTitleDashboard = () => {
  ...
  // Save recents on change
  useEffect(() => {
    if (recentActivity.length > 0) {
      localStorage.setItem('project_dashboard_recents', JSON.stringify(recentActivity));
  ...
  const handleProjectSelect = (projectIdOrObj) => {
    const id = typeof projectIdOrObj === 'object' ? projectIdOrObj.id : projectIdOrObj;
    const selectedProject = projects.find(p => p.id === id || p.name === id);
    ...
    if (selectedProject) {
      ...
      addToRecents({
        ...
        path: `/dashboard/projects?projectId=${selectedProject.id}`,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Header with navigation - Only show if a project is active */}
        {activeProject && (
          <div style={{
            backgroundColor: '#1e3a5f',
            color: 'white',
        ...
        {!activeProject ? (
          ...
          <div className="dashboard-landing-container">
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between mb-10 pt-6 gap-6">
              {/* Greeting Section */}
              <div className="flex flex-col items-start">
                <h1 className="text-[2.2rem] font-extrabold text-[#1e3a5f] mb-1 tracking-tight">
                  {getGreeting()}, {user?.full_name?.split(' ')[0] || 'User'}!
                </h1>
                <p className="text-gray-500 text-[1.05rem] font-medium">Here's what's happening with your projects today.</p>
              </div>
              ...
              <div className="w-full xl:w-[450px]">
                <div className="search-bar-wrapper">
                  ...
                  <input
                    ...
                    placeholder="Search projects, modules, or tasks..."
            ...
            <div className="modules-section">
              <div className="section-label">Quick Access Modules</div>
              <div className="modules-grid">
                {[
                  { title: 'Project Analytics', desc: 'Real-time performance metrics', icon: <BarChart3 className="h-6 w-6" />, path: '/dashboard/projects' },
                  { title: 'Upload Trackers', desc: 'Sync project data files', icon: <FileUp className="h-6 w-6" />, path: '/dashboard/trackers' },
                  { title: 'Master Data', desc: 'Manage global definitions', icon: <Database className="h-6 w-6" />, path: '/dashboard/masters' },
                  { title: 'MOM Console', desc: 'Minutes of Meeting & AI Sync', icon: <MOMIcon className="h-6 w-6" />, path: '/dashboard/mom' },
                  { title: 'System Settings', desc: 'Platform configurations', icon: <SettingsIcon className="h-6 w-6" />, path: '/dashboard/settings' },
                ].map((module, i) => (
                  <div key={i} className="module-card" onClick={() => {
                    addToRecents({
                      id: module.path,
                      type: 'module',
                      label: module.title,
                    ...
                    <div className="module-info">
                      ...
                      <div className="module-description">{module.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="projects-section">
              <div className="projects-header relative mb-0">
                <div className="section-label">Active Projects</div>
                <button 
                  className="absolute right-0 top-1/2 -translate-y-1/2 p-2 rounded border border-gray-200 text-gray-400 hover:text-[#1e3a5f] hover:border-[#1e3a5f] transition-all"
              ...
              <div style={{
                ...
                {filteredProjects.length === 0 ? (
                  <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '60px', color: 'var(--text-tertiary)', fontSize: '0.9rem' }}>
                    <Search className="h-10 w-10 mx-auto mb-4 opacity-20" />
                    <div>No projects found matching your search.</div>
                  </div>
                  ...
                  <PremiumProjectCard
                    ...
                    isFeatured={project.dashboardConfig && project.id === projects.find(p => p.dashboardConfig)?.id}
                  />
                ))}
              </div>
            </div>
