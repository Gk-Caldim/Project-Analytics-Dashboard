...
// ── BACKUP: Zoho-inspired split-screen login (previous version) ──
// Restored from: LoginPage.jsx (pre-card-redesign)
// To restore: copy this file back to LoginPage.jsx
          ...
          {!showRequestForm ? (
            ...
            <>
              ...
              <form className="ws-login-form" onSubmit={handleRequestAccess}>
                <div className="ws-input-group">
                  <label>Name</label>
                  <input type="text" placeholder="Full Name" value={reqFormData.name} onChange={(e) => setReqFormData({ ...reqFormData, name: e.target.value })} required />
                </div>
                <div className="ws-input-group" style={{ marginTop: '12px' }}>
                  <label>Email</label>
                  <input type="email" placeholder="name@company.com" value={reqFormData.email} onChange={(e) => setReqFormData({ ...reqFormData, email: e.target.value })} required />
                </div>
                <div className="ws-input-group" style={{ marginTop: '12px' }}>
                  <label>Requested Role</label>
                  <input list="roles-list" value={reqFormData.role} onChange={(e) => setReqFormData({ ...reqFormData, role: e.target.value })} placeholder="Select or type a role" required style={{ width: '100%', padding: '12px', borderRadius: '6px', border: '1px solid rgba(0,0,0,0.1)', fontSize: '14px', outline: 'none' }} />
                  <datalist id="roles-list">{roles.map(r => <option key={r.id} value={r.name} />)}</datalist>
                </div>
                <div className="ws-input-group" style={{ marginTop: '12px' }}>
                  <label>Password</label>
                  <div className="ws-password-wrapper">
                    <input type={showReqPassword ? "text" : "password"} placeholder="••••••••" value={reqFormData.password} onChange={(e) => setReqFormData({ ...reqFormData, password: e.target.value })} required />
                    <button type="button" className="ws-password-toggle" onClick={() => setShowReqPassword(!showReqPassword)}>{showReqPassword ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                  </div>
                </div>
                <div className="ws-input-group" style={{ marginTop: '12px', marginBottom: '16px' }}>
                  <label>Confirm Password</label>
                  <div className="ws-password-wrapper">
                    <input type={showReqConfirmPassword ? "text" : "password"} placeholder="••••••••" value={reqFormData.confirm_password} onChange={(e) => setReqFormData({ ...reqFormData, confirm_password: e.target.value })} required />
                    <button type="button" className="ws-password-toggle" onClick={() => setShowReqConfirmPassword(!showReqConfirmPassword)}>{showReqConfirmPassword ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                  </div>
                </div>

                {reqError && (<div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '8px', padding: '10px 14px', marginBottom: '12px' }}><p style={{ color: '#dc2626', fontSize: '13px', margin: 0 }}>{reqError}</p></div>)}
                {reqSuccess && (<div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', padding: '10px 14px', marginBottom: '12px' }}><p style={{ color: '#166534', fontSize: '13px', margin: 0 }}>{reqSuccess}</p></div>)}
